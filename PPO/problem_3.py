...
from p3_agent import RandomAgent, PPOAgent
...
#agent = RandomAgent(dim_action)
agent = PPOAgent(dim_action, dim_state, disc, lr_critic, lr_actor, epsilon)
...
for i in EPISODES:
    ...
    while not done:
        ...
        # Take action. Returns (next_state, reward, done, extra_info)
    ...
    # Train M epochs
    if type(agent) is PPOAgent:
        agent.train(buffer, epochs)
